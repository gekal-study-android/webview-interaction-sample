...
- `web/app/vconsole-loader.tsx`: vConsole（WebView 上のログ/ネットワーク確認）。有効・無効はビルド variant（`app/configs/*.json` の `vconsole` → `BuildConfig.VCONSOLE` → URL の `?vconsole=`）で決める。動的読込
...
3. **設定の分離**: URL や vConsole の有効可否などの環境依存値は `app/configs/*.json` を通じて `BuildConfig` に反映させる仕組みを利用してください。
