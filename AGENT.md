...
- `web/app/bridge-provider.tsx`: JS ⇄ Native ブリッジの状態管理（呼び出し・受信・ログ）。やり取りは画面のイベントログに加え `[Bridge]` 付きで `console.*` にも出力（vConsole 用）
...
- `web/app/vconsole-loader.tsx` / `web/app/vconsole-config.ts`: vConsole（WebView 上のログ/ネットワーク確認）。有効・無効は Web のビルド時フラグ `NEXT_PUBLIC_VCONSOLE`（`?vconsole=1`/`0` で上書き可）。動的読込
